...
document.getElementById("start-btn").addEventListener("click", () => {
  const examSpinner = document.getElementById("loader");
  examSpinner.classList.remove("d-none");
  const difficulty = localStorage.getItem("examDifficulty");
  const examName = localStorage.getItem("examName");
  fetch(`../shared/data/${examName}.json`)
    ...
    .then((data) => {
      ...
      document.getElementById("ready-sector").classList.add("d-none");
    ...
    .catch((error) => {
      ...
      location.assign("../users/login/login.html");
    }).finally(() => examSpinner.classList.add("d-none"));
});
